...
const updateStatus = async (order) => {
  const nextStatus = order.status === 'CREATED' ? 'PENDING' : 'SHIPPED'
  try {
...
<template>
  <div class="app-container">
    ...
    <div v-if="connectionError" class="error-banner">
      ...
      <strong>Backend Unreachable:</strong> Could not connect to the API. Ensure the Database and Kafka are healthy.
    </div>
    ...
    <div class="dashboard-grid">
      ...
      <div class="glass-panel">
        <div class="panel-header">
          ...
          <button type="button" @click="fetchSource" style="background: rgba(255,255,255,0.1)">Refresh</button>
        </div>
            ...
            <tr v-for="order in sourceRecords" :key="order.id">
              ...
              <td>
                <div class="action-bar">
                  <button type="button" @click="updateStatus(order)" style="padding: 4px 8px; font-size: 0.75rem;">Advance</button>
                  <button type="button" class="danger" @click="deleteOrder(order.id)" style="padding: 4px 8px; font-size: 0.75rem;">Delete</button>
                </div>
              </td>
            </tr>
      ...
      <div class="glass-panel">
        <div class="panel-header">
          ...
          <button type="button" @click="fetchIndex" style="background: rgba(255,255,255,0.1)">Refresh</button>
        </div>
